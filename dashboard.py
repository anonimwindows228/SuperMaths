...
def show_dashboard(cookie_manager):
    ...
    with t1:
        ...
        st.markdown(f"""
            <div class="stat-card">
                ...
                <div style="font-size:0.82em;color:#6b7280;margin-top:4px">{motivational(daily_answered)}</div>
            </div>""", unsafe_allow_html=True)
    with t2:
        ac  = "num-red" if daily_acc < 50 else "num-yellow" if daily_acc < 75 else "num-green"
        sub = f"From {daily_answered} questions" if daily_answered else "No questions yet today"
        st.markdown(f"""
            <div class="stat-card">
                <div class="stat-label">Today's Accuracy</div>
                <div class="{ac}">{daily_acc}%</div>
                <div style="font-size:0.82em;color:#6b7280;margin-top:4px">{sub}</div>
            </div>""", unsafe_allow_html=True)
    ...
    cols          = st.columns(len(badge_keys))
    for col, key in zip(cols, badge_keys):
        b          = BADGES[key]
        earned     = key in earned_badges
        locked     = "" if earned else "badge-locked"
        earned_cls = "badge-card-earned" if earned else ""
        with col:
            st.markdown(f"""
                <div class="badge-card {earned_cls} {locked}">
                ...
                if st.button(f"Quiz {qn} →", key=f"start_{topic}",
                    ...
                    for k in ["assign_index", "assign_score", "assign_answered",
                        ...
                        st.session_state[k] = (
                            ...
                            else False if k != "assign_question"
                            else None
                        )
                    st.rerun()
    ...
    with col_lt:
        if st.button("🎯 Take Level Test", use_container_width=True):
    ...
    vcols = st.columns(len(topics))
    for col, topic in zip(vcols, topics):
        with col:
